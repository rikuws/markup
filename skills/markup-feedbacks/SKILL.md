...
2. For each selected bundle, read `instruction.md` and `metadata.json`. Inspect `screenshot.png`, `screenshot-original.png`, and any `captures` entries returned by the list script. Use per-area notes and any `Visible UI text` / `visibleText` lines as extra context for labels in the marked region.
