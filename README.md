...
| Talk while you mark | Speak while you draw. Dictation defaults to Apple’s on-device `SpeechTranscriber` (the Notes engine) and reranks technical terms such as UI, UX, and SwiftUI. A prototype Parakeet TDT engine (FluidAudio/CoreML) can be selected in Settings for stop-to-text comparison. A live waveform shows that Markup is hearing you, including while you drag. Speech from the moment you start a new drag lands only in that area (click an area to retarget). |
